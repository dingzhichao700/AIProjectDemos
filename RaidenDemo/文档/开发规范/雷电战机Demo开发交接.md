...
- 精英敌机统一显示跟随机身移动的 HUD 血条，形式固定为轻微弯曲的细长曲线：黑色背景、红色生命填充；血条随当前生命值刷新，并在精英敌机进入死亡表现时移除。普通敌机不显示该血条，Boss 继续使用战斗界面顶部的 Boss 血条。
- 击败 Boss 才判定关卡胜利。
- 完整消灭普通敌机编队后掉落奖励；有敌机逃离则该编队不掉落。
- 当前关卡道具类型包括：飞机升级、僚机、回血、增加生命、弹药补给、能量碎片补给包（小）、能量碎片补给包（大）。能量碎片及主动技能的完整业务仍待后续接入。
- 关卡道具统一使用单一圆形拾取碰撞，配置表仅保留 `collisionRadius`，具体半径由各道具配置决定。
- 道具生成后按受约束的随机初始角度和配置速度直线运动；可反弹阶段碰到视窗边缘会反弹，配置的反弹期结束后保持最后方向运动并允许离开视窗。
- 反弹期结束前最后 8 秒使用透明度均匀往返变化提示即将离场，不使用强烈显隐闪烁；该 8 秒属于当前玩法固定规则，不进配置表。
- 道具不再吸附；玩家飞机碰撞形状与道具圆形直接接触后触发拾取。
- 拾取后隐藏图标、播放特效 `12008`，并在 HUD 层播放统一飘字；道具逻辑移动不立即停止，而是保持原方向并把速度调整为 100，待拾取表现完成后回收。
- 道具可叠加循环特效，播放比例统一为 1.25；具体特效 ID 读取 `StageItemResource`。
...
- 关卡道具已完成圆形碰撞、随机方向移动、视窗反弹、到期透明度提示、拾取特效和 HUD 飘字的基础闭环；弹药补给、能量碎片及主动技能的完整效果逻辑仍待开发。
- TMP 全局字体、Fallback、材质预设及现有 Prefab/场景引用已经迁移并人工验证；旧 LiberationSans 和旧 `msjhbd SDF` 已清理。
